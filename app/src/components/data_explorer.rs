...
use crate::components::overview_table::{OverviewSort, OverviewTable, OverviewTotals};
...
pub fn DataExplorerPanel(mut props: DataExplorerProps) -> Element {
    ...
    let overview_sort_col = use_signal(OverviewSort::default);
    let overview_sort_asc = use_signal(|| false);
                    ...
                    if matches!(view_mode(), ViewMode::CombatLog) {
                        ...
                        div { class: "overview-section",
                            ...
                            // Overview table - uses memoized data
                            {
                                ...
                                rsx! {
                                    // PvP encounters split into friendly/enemy tables; PvE gets one
                                    {
                                        if let Some((friendly_totals, enemy_totals)) = table_data.pvp_totals.clone() {
                                            let side_rows = |f: Option<PvpFaction>| -> Vec<RaidOverviewRow> {
                                                table_data.rows.iter().filter(|r| r.faction == f).cloned().collect()
                                            };
                                            let unknown = side_rows(None);
                                            rsx! {
                                                OverviewTable {
                                                    side_label: Some(("friendly", "Friendly")),
                                                    rows: side_rows(Some(PvpFaction::Friendly)),
                                                    total_rows: vec![("Friendly Total", friendly_totals)],
                                                    sort_col: overview_sort_col,
                                                    sort_asc: overview_sort_asc,
                                                    european: eu,
                                                }
                                                OverviewTable {
                                                    side_label: Some(("enemy", "Enemy")),
                                                    rows: side_rows(Some(PvpFaction::Enemy)),
                                                    total_rows: vec![("Enemy Total", enemy_totals)],
                                                    sort_col: overview_sort_col,
                                                    sort_asc: overview_sort_asc,
                                                    european: eu,
                                                }
                                                if !unknown.is_empty() {
                                                    OverviewTable {
                                                        side_label: Some(("unknown", "Unclassified")),
                                                        rows: unknown,
                                                        total_rows: Vec::new(),
                                                        sort_col: overview_sort_col,
                                                        sort_asc: overview_sort_asc,
                                            ...
                                            rsx! {
                                                OverviewTable {
                                                    ...
                                                    total_rows: vec![("Group Total", table_data.totals.clone())],
                                                    sort_col: overview_sort_col,
                                                    sort_asc: overview_sort_asc,
                                                    european: eu,
                                                }
